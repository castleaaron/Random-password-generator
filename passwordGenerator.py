import random
class Password:
    def replaceLetters(self, password) -> str:
        '''
    ...
    def capOrNot(self, password) -> str:
        '''
        ...
        for i in range(len(words)):#go through each word
            while True:
                ...
                if randNum in usedWords:#if already used try again
                    ...
                    break
        return self.replaceLetters(self.capOrNot(password))
...
passw = Password()
passw.main()
input('Thank you! - Click return to exit.')
